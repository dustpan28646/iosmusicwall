...
void callHungarian(int *retCost, int rows, int cols, int costMatrix[rows][cols], int assignedIndices[rows])
{
    
    /* an example cost matrix */
//    int r[3*2] =  {  1000, 1000, 1000,
//        100, 1000, 2};
//    int** m = array_to_matrix(r,3,2);
    
    hungarian_problem_t p;
    
    /* initialize the gungarian_problem using the cost matrix*/
    int matrix_size = hungarian_init(&p, rows, cols, costMatrix, HUNGARIAN_MODE_MINIMIZE_COST);
    ...
    hungarian_solve(&p, retCost, rows, assignedIndices);
    ...
    /*
     ...
     printf("%i ",doShareInstrument(i, j));
     if (j==6)
     {
     printf("]\n");
     }
...
int hungarian_init(hungarian_problem_t* p, int rows, int cols, int cost_matrix[rows][cols], int mode) {
...
void hungarian_solve(hungarian_problem_t* p, int *retCost, int rows, int assignedIndices[rows])
{
